...
Write a function to get no of employees under each manager in the hierarchy not just their direct
 reports. It may be assumed that an employee directly reports to only one manager. In the above
 dictionary the root node/ceo is listed as reporting to himself.
...
A trick in step 2.b is to use memorization(Dynamic programming) while finding number of employees under
 a manager so that we don’t need to find number of employees again for any of the employees. In the
 below code populateResultUtil() is the recursive function that uses memoization to avoid re-computation
 of same results.*/

public class NumOfEmpUnderManager {
        ...
        private static void populateResult(Map<String, String> dataSet)
        {
            ...
            for (Map.Entry<String,String> entry: dataSet.entrySet())
            {
                ...
                if (!emp.equals(mngr)) // excluding emp-emp entry
                {
                    ...
                    List<String> directReportList = mngrEmpMap.computeIfAbsent(mngr,
                            k -> new ArrayList<>());
