...
hundreds etc.. form, each index of the string will have some fixed occurrence. The 1st index
 will have
...
Now, to handle the multiplication we will be having a multiplying factor which starts from 1.
It’s clear
from the example that the multiplying factor(in reverse) is 1, 11, 111, … and so on. So the
multiplication
...
public class SumAllSubStringsNumber {
    ...
    public static long sumOfSubstrings(String num)
    {
        ...
        // Here traversing the array in reverse order.Initializing loop from last
        // element. mf is multiplying factor.
        long mf = 1;
        for (int i = num.length() - 1; i >= 0; i --)
        {
            // Each time sum is added to its previous sum. Multiplying the three factors as
            // explained above. s[i]-'0' is done to convert char to int.
