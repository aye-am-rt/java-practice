...
/**A simple solution is to traverse the input array. As soon as we find a 0, we return n – index
 * of first 0. Here n is number of elements in input array. Time complexity of this
...
public class FindNumOfZeros {
    ...
    private static String countZeroes(int[] arr, int n) {
        int left=0;
        int right=n-1;
        while(left<right)
        {
            ...
            if(arr[mid]==1 && arr[mid+1]==0)
                return String.valueOf(n-mid-1);
            else if(arr[mid]==1 && arr[mid+1]==1)
                left=mid;
            else
                right=mid;
        }
        return "********* No zeros Present *********";
    }
